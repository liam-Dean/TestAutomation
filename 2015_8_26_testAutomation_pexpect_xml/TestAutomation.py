...
if __name__== '__main__':
    ...
    '''
    cbm1    = CBM1(condition1, 'CBM1',CBM["hostname"],CBM["username"],CBM["password"],CBM["port"])      #create two thread within a condition lock
    #server  = Server(condition1,'Server',SERVER["hostname"],SERVER["username"],SERVER["password"],SERVER["port"],fileDirection,0)
    server.start()  #server start first but the thread will set to wait when acquire the lock
    cbm1.start()    #Though start later but the thread will finish its processing first
    cbm1.join()
    server.join()   #Main thread wait until the thread above finish their tasks
    ...
    #cbm2    = CBM2(condition2, 'CBM2',CBM["hostname"],CBM["username"],CBM["password"],CBM["port"])         #create two thread within a condition lock
    #core    = Core(condition2,'Core',CORE["hostname"],CORE["username"],CORE["password"],CORE["port"])
    '''