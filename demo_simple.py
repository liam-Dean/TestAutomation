...
if len(sys.argv) > 1:
    ...
    hostname = "192.168.191.142" #input('Hostname: ')
if len(hostname) == 0:
...
if username == '':
    ...
    username = "jason" #input('Username [%s]: ' % default_username)
    if len(username) == 0:
        username = default_username
if not UseGSSAPI or (not UseGSSAPI and not DoGSSAPIKeyExchange):
    password ="996633" #getpass.getpass('Password for %s@%s: ' % (username, hostname))
