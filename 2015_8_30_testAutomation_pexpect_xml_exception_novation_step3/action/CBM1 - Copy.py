...
class CBM1(threading.Thread):
    ...
    def check_config(self):
        ...
        try:
            ...
            Channel.sendLine('billmtc');        Channel.waitUntil(self.timeout,'BILLMTC');     
            Channel.waitUntil(self.timeout,'18 Refresh')
            Channel.send('schedule');           
            Channel.waitUntil(self.timeout,'schedule');   
            Channel.waitUntil(self.timeout,'18 Refresh')
            Channel.send('RTB');               
            Channel.waitUntil(self.timeout,'RTB');         
            Channel.waitUntil(self.timeout,'18 Refresh')
            Channel.send('Query AMA');          
            Channel.waitUntil(self.timeout,'18 Refresh');  
            Channel.waitUntil(self.timeout,'----------------------------------------------------')
